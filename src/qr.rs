...
/// Guarantees 100% font-independent compatibility across all hardware/framebuffer consoles.
pub fn render_full_block_qr(data: &str) -> Result<Vec<Line<'static>>, String> {
    ...
    let width = qr.width();
    let quiet = 1; // Exactly 1-module white quiet zone ring
    let total_w = width + quiet * 2;
    let mut lines = Vec::new();

    // Top quiet zone (1 row of white)
    ...
    for y in 0..width {
        ...
        // Left quiet zone (1 module = 2 spaces)
        ...
        // Right quiet zone (1 module = 2 spaces)
        spans.push(Span::styled("  ", Style::default().bg(Color::White)));

        lines.push(Line::from(spans));
    }

    // Bottom quiet zone (1 row of white)
    lines.push(Line::from(Span::styled(
        " ".repeat(total_w * 2),
        Style::default().bg(Color::White),
    )));
...
/// Render half-block QR using Unicode half-block characters (▀ and ▄).
/// Encodes TWO vertical modules per text line.
/// On standard 6x12 console fonts (Terminus ter-v12n), 1 horizontal char (6px)
/// by 0.5 vertical line (6px) renders each module as an exact, distortion-free 6x6 pixel square!
/// Total vertical height is cut in half (20-25 lines), guaranteeing 100% fit on all console screens without bottom clipping.
pub fn render_half_block_qr(data: &str) -> Result<Vec<Line<'static>>, String> {
    ...
    let width = qr.width();
    let quiet = 2; // 2-module white quiet zone
    let total_w = width + quiet * 2;
    ...
    while y < total_h {
        let mut spans = Vec::new();
        // Left margin padding for clean console centering
        spans.push(Span::raw("    "));

        for x in 0..total_w {
            let top_dark = if x < quiet || x >= width + quiet || y < quiet || y >= width + quiet {
                false
            } else {
                qr[(x - quiet, y - quiet)] == QrColor::Dark
            };

            let next_y = y + 1;
            let bot_dark = if x < quiet || x >= width + quiet || next_y < quiet || next_y >= width + quiet {
                false
            } else {
                qr[(x - quiet, next_y - quiet)] == QrColor::Dark
            };

            let span = match (top_dark, bot_dark) {
                (true, true) => Span::styled(" ", Style::default().bg(Color::Black)),
                (true, false) => Span::styled("▀", Style::default().fg(Color::Black).bg(Color::White)),
                (false, true) => Span::styled("▄", Style::default().fg(Color::Black).bg(Color::White)),
                (false, false) => Span::styled(" ", Style::default().bg(Color::White)),
            };
            spans.push(span);
        }
        lines.push(Line::from(spans));
